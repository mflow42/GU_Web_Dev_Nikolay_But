...
const cart = {
  ...
  render() {
    ...
    if (this.items.length > 0) {
      ...
      //если вещей нет, то показываем пустую корзину
  ...
  makeForm() {
    ...
    const textareaHandleHeight = (inputTextArea, rowsDefault) => {
      ...
      const stretchHeight = function () {
        ...
        if (counter <= rowsDefault) return;else if (counter > rowsDefault) this.rows = counter;
      };
  ...
  render(commentEl, comments, settingsComments) {
    ...
    if (comments.length > 0) {
      for (const obj of comments) {
        const comment = new Comment(obj.id_comment, obj.text, obj.approved, `${settingsComments.idContainer}${settingsComments.classComment}`, `${settingsComments.classCommentApproved}`, `${settingsComments.idContainer}${settingsComments.classMessage}`, `${settingsComments.idContainer}${settingsComments.classBtnDelete}`, `${settingsComments.idContainer}${settingsComments.classBtnApprove}`);
